// Package llm provides a thin client for an OpenAI-compatible endpoint.
package llm
...
func (c *Client) ask(prompt, question, context string) (string, error) {
	...
	// Token caps: separate Ask vs AskCase with env overrides
	maxTokens := 900
	if !isAsk { // AskCase default
		maxTokens = 450
	}
	if isAsk {
		if v := os.Getenv("LLM_ASK_MAX_TOKENS"); v != "" {
